...
const NavItem = memo(({ path, name, icon, badge }) => {
  ...
  return (
    <li className="px-3 mb-1 text-black">
      <NavLink
        to={path}
        className={`
        ...
        `}
      >
        <i className={`${icon} text-lg ${isActive ? 'text-white' : 'text-black group-hover:text-black'}`} />
        <span className="ml-3 font-medium">{name}</span>
        {badge && (
          <span className={`
            ml-auto px-2 py-1 text-xs rounded-full font-bold
            ${isActive ? 'bg-white text-black' : 'bg-gray-300 text-white'}
          `}>
            {badge}
          </span>
        )}
...
const NavigationSection = memo(({ title, items }) => (
  <div className="mb-6 hidebar">
    <h6 className="px-6 text-xs font-bold text-black uppercase tracking-wider mb-4">
      {title}
    </h6>
    <ul>
      {items.map(item => (
  ...
  return (
    <>
      <nav className={`
        ...
        md:w-72 bg-white text-black shadow-lg
        ...
        <div className="md:flex-col md:items-stretch md:min-h-full md:flex-nowrap px-0 flex flex-wrap items-center justify-between w-full mx-auto">
          ...
          <div className="px-6 py-4 mb-6">
            <NavLink to="/" className="flex items-center">
              ...
              <span className="ml-3 text-xl font-bold text-black">Your Brand</span>
            </NavLink>
          </div>
      ...
      \
          ...
          {/* <div className="px-6 mt-auto">
            <div className="bg-[#FFDBB5] rounded-xl p-4 mb-6">
              <h6 className="text-sm font-semibold text-black mb-2">Need Help?</h6>
              <p className="text-xs text-black mb-3">Contact our support team</p>
              <button className="w-full bg-black text-white text-sm font-medium px-4 py-2 rounded-lg hover:bg-black transition-colors">
                Contact Support
              </button>
            </div>
          </div> */}
